module Api::V2::LookupKeysCommonController
  ...
  def return_if_smart_mismatch
    if (@smart_class_parameters && @smart_class_parameter &&
       !@smart_class_parameters.find_by_id(@smart_class_parameter.id)) ||
        (@smart_class_parameters && !@smart_class_parameter)

      not_found "Smart class parameter not found by id '#{params['id']}'"
    end
  end
